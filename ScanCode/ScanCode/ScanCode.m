...
- (void)setQRCodeFrameView
{
    ...
    CGRect cropRect = CGRectMake((size.width - 220) * 0.5, (size.height - 220) * 0.5, 220, 220);
    ...
    borderView.center = bgViwe.center;
    borderView.bounds = CGRectMake(0, 0, 230, 230);
    UIImage *borderImage = [UIImage imageNamed:@"pick_bg@2x.png"];
    ...
    [bgViwe addSubview:borderView];
    
    // 扫描线
    _scanLine = [[UIImageView alloc] initWithFrame:CGRectMake((size.width - 220) * 0.5, (size.height - 220) * 0.5, 220, 2)];
    _scanLine.image = [UIImage imageNamed:@"line.png"];
    [bgViwe addSubview:_scanLine];
    
    NSLog(@"_scanLine:%f",borderView.frame.origin.y + borderView.frame.size.height + 10);
    
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake((size.width - 220) * 0.5,borderView.frame.origin.y + borderView.frame.size.height + 10, 220, 30)];
